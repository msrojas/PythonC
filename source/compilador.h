...
extern uint8_t list_existe;

enum {VARIABLE=1,CADENA,NUMERO,IGUAL,OPERADOR,PRINT,R_PARENTESIS,L_PARENTESIS,OPE_DE_PARENTESIS,CONCATENACION,PLUS_CONCAT,CHAR,FLOAT,RAW_INPUT,INT,FLOAT_F,LEN,IF,MAYOR_QUE,MENOR_QUE,DOS_PUNTOS,ELSE,DISTINTO,AND,OR,NOT,STRCMP,ELIF,WHILE,R_CORCHETES,L_CORCHETES,CHAR_CORCHETES,FREE,ZERO,LIST,FOR,IN,RANGE,GET_INDICE_INT,GET_INDICE_FLOAT,GET_INDICE_CHAR}tokens;
